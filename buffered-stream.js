// the basis for this is https://github.com/rvagg/node-levelup/blob/master/lib/read-stream.js
...
var Readable      = require('readable-stream').Readable
  ...
  , EncodingError = require('levelup-util/errors').EncodingError
...
ReadStream.prototype._read = function read () {
  ...
  self._iterator.binding.next(function(err, array) {
    if (err || (array[array.length - 1] === null)) {
      if (!err && !self._destroyed) {
        array.length = array.length - 1
        process.nextTick(function () {
          // must do some cleanup before pushing some more data
          self.push(array)
          self.push(null)
        })
      }
    ...
    try {
      array = array.map(function (obj) {
        return self._makeData(obj.key, obj.value)
      })
    } catch (e) {
